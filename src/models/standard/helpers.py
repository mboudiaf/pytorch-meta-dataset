...
def load_custom_pretrained(model, default_cfg=None, load_fn=None, progress=True, check_hash=False):
    ...
    if not pretrained_url:
        logger.warning("No pretrained weights exist for this model. Using random initialization.")
        return
    ...
    if load_fn is not None:
        ...
        logger.warning("Valid function to load pretrained weights is not available, using random initialization.")
...
def load_pretrained(model, default_cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=True, progress=False):
    ...
    if pretrained_url:
        ...
        logger.info(f'Loading pretrained weights from Hugging Face hub ({hf_hub_id})')
